"""
ULTIMATE REAL-TIME SECURITY COMMAND CENTER - COMPLETE
BBIT Final Year Project - All Modules Working
        ...
        for i in range(0, len(theme_items), 3):
            ...
            for j in range(3):
                if i + j < len(theme_items):
                    name, colors = theme_items[i + j]
                    with cols[j]:
                        if st.button(name, key=f"quick_theme_{i+j}", use_container_width=True):
...
def live_dashboard():
    ...
    with col1:
        ...
        fig = go.Figure(go.Indicator(
            ...
            value=metrics['cpu'],
            domain={'x': [0, 1], 'y': [0, 1]},
            title={'text': 'CPU %', 'font': {'color': 'white'}},
            gauge={
    ...
    with col2:
        ...
        fig = go.Figure(go.Indicator(
            ...
            domain={'x': [0, 1], 'y': [0, 1]},
            title={'text': 'Memory %', 'font': {'color': 'white'}},
            gauge={
                'axis': {'range': [None, 100], 'tickcolor': 'white'},
